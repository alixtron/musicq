...
module.exports = {
  name: "move",
  aliases: ["mv", "nvs"],
  description: `avaz kardan jay song dar playlist`,
  ...
  run: async (client, message, args, prefix, queue) => {
    ...
    if (songIndex > queue.songs.length - 1) {
      return client.embed(
        message,
        ` **akharin song playlist: \`${queue.songs.length}\`**`
      );
    } else if (position === 0) {
      return client.embed(message, `**ghabl az play kardan song nemitoni move bedi**`);
    } else {
